...
bool platform_create_window(int width, int height, wchar_t* title) {
  HINSTANCE intance = GetModuleHandle(0);

  WNDCLASS wc      = {};
  wc.hInstance     = intance;
  wc.hIcon         = LoadIcon(intance, IDI_APPLICATION);
  ...
  window = CreateWindowEx(0, title, title, dwStyle, 100, 100, width, height,
                          NULL, NULL, intance, NULL);
  ...
  // Fake Window initializing OpenGL
  {
    ...
    wglChoosePixelFormatARB =
        (PFNWGLCHOOSEPIXELFORMATARBPROC)platform_load_gl_function(
            "wglChoosePixelFormatARB");
    wglCreateContextAttribsARB =
        (PFNWGLCREATECONTEXTATTRIBSARBPROC)platform_load_gl_function(
            "wglCreateContextAttribsARB");
    if (!wglCreateContextAttribsARB || !wglChoosePixelFormatARB) {
      SM_ASSERT(false, "Failed to load OpenGL functions pixel/context");
      return false;
    }
    ...
    // Cannot reuse smae DC because "SetPixelFormat"
    DestroyWindow(window);
  }

  // ACTUAL Window OpenGL initialization
  {
    // border size of window
    {
      RECT borderRect = {};
    ...
    const int pixelAttribs[] = {
        ...
        WGL_DEPTH_BITS_ARB,
        24,
        0  // Terminate with 0 so OpenGL doesn't throw Error!
    };

    UINT numPixelFormats;
    int  pixelFormat = 0;
    if (!wglChoosePixelFormatARB(dc, pixelAttribs, 0, 1, &pixelFormat,
                                 &numPixelFormats)) {
    ...
    if (!rc) {
      SM_ASSERT(0, "Failed to create OpenGL Render Context");
      return false;
    }
...
void platform_swap_buffers() { SwapBuffers(dc); }
